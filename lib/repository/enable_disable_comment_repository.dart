import 'package:bettersolver/models/common_message_model.dart';
import 'package:bettersolver/utils/apiprovider.dart';
import 'package:shared_preferences/shared_preferences.dart';

class EnableDisableCommentRepository {
  ApiProvider _provider = ApiProvider();

  Future<CommonMessageModel> fetchenabledisablecomment(
      String postid, String type) async {
    SharedPreferences pref = await SharedPreferences.getInstance();
    String? s = pref.getString('s');
    String? userid = pref.getString('userid');

    final requestBody = {
      "user_id": userid,
      "s": s,
      "post_id": postid,
      "type": type,
    };

    final response = await _provider.httpMethodWithoutToken(
      'post',
      'demo2/app_api.php?application=phone&type=disable_comment',
      requestBody,
    );

    return CommonMessageModel.fromJson(response);
  }
}
